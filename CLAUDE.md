...
- **Quota tracking (Boss-mandated 2026-09-07): every narration run prints the account quota before and after** (tier, characters used of the period limit, remaining, reset date) and the observed account usage change over the run (an interval measurement of the account, not proof of this essay's charge: anything else using the key in the window lands in it); a run that needs more than what remains refuses before calling TTS; a run that spends and then fails still writes a `failed-run` ledger line. Read it on demand with `python3 -c` against `/v1/user/subscription` or by running the generator with `--cache-only` plus a real key lookup. Report the remaining balance and reset date to the Boss whenever a narration is proposed. **Per-transmission ledger:** every run appends one JSON line to `narration-ledger.jsonl` (repo root; slug, chunks and characters requested, the account delta actually billed, body chars, words, minutes, quota after). July narrations are backfilled as estimates from today's text; 2026-09-07 runs as measured. Use the ledger to predict: 2026-09-07 evidence says ElevenLabs bills about 1:1 on requested characters for full runs and less on some partial runs (075 v5: 7,584 requested, 5,717 billed). Snapshot 2026-09-07 19:50: Creator tier, 33,572 of 265,358 used, resets 2026-09-21 23:45.
